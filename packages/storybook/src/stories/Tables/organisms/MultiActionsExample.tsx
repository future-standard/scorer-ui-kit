...
const rowMaker = (
  rowData: ITableSampleData[],
  generateConfigButtons: (id: string) => IconButtonData[]
): ITypeTableData => {
  const newRows: ITypeTableData = rowData.map(({ id, created, totalTime, usage, usageUnit }) => {
    const row: IRowData = {
      id,
      header: {
        image: photo,
...
const getFilteredData = (
  currentSelected: IFilterResult[],
  data: ITableSampleData[]
): ITableSampleData[] => {
  if (Array.isArray(currentSelected) && currentSelected.length > 0) {
    const filteredData: ITableSampleData[] = currentSelected.reduce(
      (accumulator, currentFilter) => {
        ...
        if (
          currentFilter.id === 'datePickerForRuntime' &&
          !isFilterItem(currentFilter.selected) &&
      ...
      return [
        {
          icon: 'Delete',
          onClick: () => {
            openConfirmationModal(() => handleDelete(rowId));
          },
        },
        {
          icon: 'Download',
      ...
      <TableBarWrapper>
        {visibleBar === 'filterBar' && (
          <FilterBar
            ...
            resultTextTemplate={
              language === 'english' ? resultTextTemplateEng : resultTextTemplateJp
            }
            ...
            actionButtons={[
              {
                ...
                onClick: () => {
                  openConfirmationModal(handleDeleteSelected);
                },
              },
              {
                icon: 'Download',
