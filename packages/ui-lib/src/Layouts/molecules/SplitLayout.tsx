...
import type { ISideAreaState, ISplitLayoutHandles, ISplitLayoutProps } from '..';
...
const SplitLayout = forwardRef<ISplitLayoutHandles, ISplitLayoutProps>(
  (
    {
      mainArea,
      sideArea,
  ...
  */
    ...
    }, [setSavedCollapsedState, setSavedSize, setSavedLastOpenSize, sideAreaStartBasis, persist]);
    ...
    const open = useCallback(() => {
      ...
      if (sideAreaBasis > closedBasis && AreaB.current) {
        openBasis =
          layout === 'horizontal' ? AreaB.current.clientWidth : AreaB.current.clientHeight;
      } else {
      ...
      (event: PointerEvent<HTMLDivElement>) => {
        ...
        if (resizing && initialMousePos && sideAreaStartBasis) {
          ...
          if (layout === 'horizontal') {
            ...
            if (!reverse) {
              newBasis = clampInt(
                sideAreaStartBasis + (initialMousePos.x - clientX),
                null,
                maxClamp
              );
            } else {
            ...
            if (!reverse) {
              newBasis = clampInt(
                sideAreaStartBasis + (initialMousePos.y - clientY),
                null,
                maxClamp
              );
            } else {
