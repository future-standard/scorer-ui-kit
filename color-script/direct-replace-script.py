...
class RGB:
    ...
    @staticmethod
    def from_hex(hex: HEX ):
      r = int(f'{hex.h}{hex.e}', 16)
      g = int(f'{hex.x}{hex.a}', 16)
      b = int(f'{hex.d}{hex.c}', 16)
      alpha = 1
...
## build a new variables dictionary based in another
def find_and_add_known(hsl_file, source_dict):
    with open(hsl_file, "r") as file:
        new_content = ""
        for line in file.readlines():
            new_line = line
            for key in source_dict:
                ...
                color_variable = source_dict[key]["variable"]
                var_template = f"var({color_variable}): key"

                new_line = new_line.replace(key, var_template)
            new_content += new_line

    with open(f"{hsl_file}.new.ts", "w") as file:
        file.write(new_content)
...
## Build light dictionary from another
# color_variables = build_variables_dict("./colors/light-variables.css")
# find_and_add_known("./target/old-light.css", color_variables)